...
//CHANGE PLAY BUTTON FROM PLAY TO PAUSE ICON WHEN CLICKED
function changeImg() {
    if (image.src.match("play-icon.webp")) {
        image.src = "pause-icon.webp";
        playAnimation();
        widget.play();
    } else {
        image.src = "play-icon.webp";
        widget.pause();
    }
}


//This function was created with help from an example on W3schools.com
function playAnimation() {
    if (start == 0) {
        ...
        function frame() {
            if (width >= 100 || image.src.match("play-icon.webp")) {
                clearInterval(id);
                start = 0;
            } else {
                width++;
